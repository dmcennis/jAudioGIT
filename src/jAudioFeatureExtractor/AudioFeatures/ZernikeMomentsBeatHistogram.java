...
public class ZernikeMomentsBeatHistogram extends FeatureExtractor {
	...
	public double[] extractFeature(double[] samples, double sampling_rate,
		...
		for(int i=0;i<other_feature_values.length;++i){
			for(int j=0;j<other_feature_values[0].length;++j){
				double base = Math.sqrt(Math.pow((2.0*((double)i)/((double)(other_feature_values.length)))-1.0,2.0)+Math.pow((2.0*((double)j)/((double)(other_feature_values[0].length)))-1.0,2.0));
		...
		for(int n=8;n<count;n+=1){
			for(int m=n;m>=0;m-=2){
				...
				for(int k=0;k<(n-m)/2;++k){
					...
					for(int i=0;i<other_feature_values.length;++i){
						for(int j=0;j<other_feature_values[0].length;++j){
							double radius = Math.sqrt(Math.pow((2*i/other_feature_values.length)-1.0,2.0)+Math.pow((2*j/other_feature_values[0].length)-1.0,2.0));
	...
	@Override
	public Object clone() {
		ZernikeMomentsBeatHistogram ret = new ZernikeMomentsBeatHistogram();
        ret.count = count;
